#class_name TileFace
@tool
extends "res://source/tile/tile_face.gd"
	...
	for status in tile.get_statuses():
		if status is CustomStatus and not status.face_status:
			...
			if not status.face_color.is_empty():
				face_color=status.face_color[TileType.DAMAGE%status.face_color.size()]
				set_color(face_color)

			if not status.value_color.is_empty():
				value_color = status.value_color[TileType.DAMAGE%status.value_color.size()]
