...
func get_spell_convert_from_id(spell: Spell) -> String:
	if spell.id in SpellLoader.spell_upgrades:
		return spell.id
	elif spell.secret_id in SpellLoader.spell_upgrades:
...
func track_red_letter_upgrades_found() -> void :
	...
	for other_spell in Game.player.get_spells():
		...
		if other_convert_from_id != "":
			save.track_spell_found(SpellLoader.spell_upgrades[other_convert_from_id], Game.difficulty)
...
func upgrade_spell(spell: Spell) -> void :
	...
	if AchievementManager.should_track_stats():
		...
		SaveManager.get_save().track_spell_taken(SpellLoader.spell_upgrades[convert_from_id], Game.difficulty)

	spell.transform_spell(SpellLoader.spell_upgrades[convert_from_id], not convert_from_id in Globals.GIFTS)
